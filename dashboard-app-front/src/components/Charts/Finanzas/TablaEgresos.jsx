...
const TablaEgresos = ({ egresos }) => {
    const [page, setPage] = useState(1);
    const rowsPerPage = 5;

    const totalPages = Math.ceil(egresos.length / rowsPerPage);
    const startIndex = (page - 1) * rowsPerPage;
    const currentRows = egresos.slice(startIndex, startIndex + rowsPerPage);

    return (
        <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-4">
            <h3 className="text-lg font-semibold mb-4 dark:text-white">Egresos</h3>
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                    {currentRows.map(({ id, descripcion, egresos: monto, fecha }) => (
